...
SPDIFOut::SPDIFOut(int dout_pin, int port, int dma_buf_count)
{
  ...
  if (i2s_driver_install((i2s_port_t)portNo, &i2s_config_spdif, 0, NULL) != ESP_OK) {
    ESP_LOGE(BT_AV_TAG,"ERROR: Unable to install I2S drivers");
    return;
  }
...
bool SPDIFOut::SetPinout(int bclk, int wclk, int dout)
{
  ...
  if (i2s_set_pin((i2s_port_t)portNo, &pins) != ESP_OK) {
    ESP_LOGE(BT_AV_TAG,"ERROR setting up S/PDIF I2S pins\n");
    return false;
  }
  return true;
}
...
bool SPDIFOut::SetRate(int hz)
{
  ...
  if (i2s_set_sample_rates((i2s_port_t)portNo, adjustedHz) == ESP_OK) {
    ...
    ESP_LOGE(BT_AV_TAG,"ERROR changing S/PDIF sample rate");
  } 
  return true;
}

bool SPDIFOut::SetBitsPerSample(int bits)
{
...
bool SPDIFOut::SetChannels(int channels)
{
...
bool SPDIFOut::SetOutputModeMono(bool mono)
{
  this->mono = mono;
